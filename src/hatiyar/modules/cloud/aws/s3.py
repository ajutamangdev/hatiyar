...
class Module(ModuleBase):
    ...
    def initialize_client(self) -> bool:
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "UnauthorizedOperation":
                ...
                console.print(f"[red]✗[/red] AWS Error: {e.response['Error']['Message']}")
            return False
    ...
    def _check_bucket_security(self, bucket_info: Dict[str, Any]) -> List[str]:
        ...
        if not public_access.get("enabled") or not all([
            ...
            findings.append(f"Bucket '{bucket_name}' may allow public access (public access block not fully configured)")
        ...
        for grant in acl.get("grants", []):
            ...
            if "AllUsers" in grantee_uri or "AuthenticatedUsers" in grantee_uri:
                findings.append(f"Bucket '{bucket_name}' has public ACL grants: {grant.get('permission')}")

        # Check logging
        if not bucket_info.get("logging", {}).get("enabled"):
            findings.append(f"Bucket '{bucket_name}' does not have access logging enabled")
    ...
    def enumerate_buckets(self):
        ...
        try:
            ...
            for idx, bucket in enumerate(buckets, 1):
                ...
                encryption_status = "✓ Enabled" if bucket_info["encryption"]["enabled"] else "✗ Disabled"
                encryption_color = "green" if bucket_info["encryption"]["enabled"] else "red"
                
                versioning_status = bucket_info["versioning"]["status"]
                versioning_color = "green" if versioning_status == "Enabled" else "yellow"
                ...
                public_status = "✓ Blocked" if is_fully_blocked else "⚠ Not Fully Blocked"
                public_color = "green" if is_fully_blocked else "yellow"
                
                logging_status = "✓ Enabled" if bucket_info["logging"]["enabled"] else "✗ Disabled"
                logging_color = "green" if bucket_info["logging"]["enabled"] else "yellow"
  ...
  • Encryption: [{encryption_color}]{encryption_status}[/{encryption_color}]"""
                ...
                if public_grants:
                    ...
                    for grant in public_grants[:3]:  # Show first 3
                        ...
                        grantee_type = "All Users" if "AllUsers" in grantee_uri else "Authenticated Users"
                ...
                # Website Hosting
                website_status = "✓ Enabled" if bucket_info["website"]["enabled"] else "✗ Disabled"
                ...
                # Replication
                replication_status = "✓ Enabled" if bucket_info["replication"]["enabled"] else "✗ Disabled"
                replication_color = "cyan" if bucket_info["replication"]["enabled"] else "dim"
                ...
                if self.options.get("ENUMERATE_OBJECTS") and "objects" in bucket_info:
                    ...
                    if "error" not in obj_data:
                        ...
                        if obj_data["objects"]:
                            total_size = sum(obj.get("size", 0) for obj in obj_data["objects"])
            ...
            console.print(f"[red]✗ Error enumerating buckets: {e.response['Error']['Message']}[/red]")
        except Exception as e:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        console.print(
            f"[bold]Total Buckets:[/bold] {self.data['total_buckets']}"
        )
        console.print(
        ...
        for bucket_info in self.data["buckets"]:
            if (bucket_info.get("encryption", {}).get("enabled") and
                bucket_info.get("versioning", {}).get("status") == "Enabled" and
                bucket_info.get("logging", {}).get("enabled")):
                fully_secure.append(bucket_info["name"])
        
        if fully_secure:
            console.print(f"\n[bold green]Buckets with Full Security:[/bold green]")
            for bucket_name in fully_secure:
                console.print(f"  [green]✓[/green] {bucket_name} (Encryption + Versioning + Logging)")
